...
#define MASTER_TIMEOUT_MS 2000   // 2s without PING = master down
#define HEARTBEAT_LED    2       // GPIO pin for heartbeat LED (optional)

static TickType_t last_master_ping = 0;
...
// ---------------- Master watchdog ----------------
void master_watchdog_task(void *arg) {
    ...
    while (1) {
        ...
        if ((now - last_master_ping) * portTICK_PERIOD_MS > MASTER_TIMEOUT_MS) {
            ESP_LOGW(TAG, "Master not seen for %d ms!", MASTER_TIMEOUT_MS);
            gpio_set_level(HEARTBEAT_LED, 0);  // LED off if master missing
        } else {
            ESP_LOGI(TAG, "Master alive");
            gpio_set_level(HEARTBEAT_LED, 1);  // LED on when master alive
        }
    }
}

void master_ping_received(void) {
    last_master_ping = xTaskGetTickCount();
}
...
void app_main(void) {
    ...
    // ---------------- Start watchdog task ----------------
    xTaskCreate(master_watchdog_task, "master_watchdog", 2048, NULL, 1, NULL);
    ...
    while (1) {
        ...
        int start_idx = -1;
        for (int i = 0; i < len; ++i)
            if (rxbuf[i] == START_BYTE) { start_idx = i; break; }
        ...
        if (cmd == CMD_PING) {
            master_ping_received();  // update watchdog

            if (addr == MY_ADDR) {
                float moisture = read_soil_moisture(adc_handle);

                uint8_t payload[1] = { (uint8_t)(moisture + 0.5f) };
                int txlen = 0;
