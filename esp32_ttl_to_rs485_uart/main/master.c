...
#define UART_DE_PIN CONFIG_RS485_UART_DE // DE pin
#define UART_RE_PIN CONFIG_RS485_UART_RE // RE pin if separate
...
void app_main(void) {
  ...
  while (1) {
    for (int addr = 1; addr <= N_SLAVES; ++addr) {
      ...
      int len =
          uart_read_bytes(UART_PORT, rxbuf, sizeof(rxbuf), PACKET_READ_TICKS);
      if (len >= 5 && rxbuf[0] == START_BYTE) {
        ...
        if (checksum(&rxbuf[1], 3 + r_len) == r_cs && r_cmd == CMD_PONG) {
          ...
          if (r_len >= 3) {
            int16_t temp_i16 = rxbuf[5] | (rxbuf[6] << 8);
            float tempC = temp_i16 / 100.0f;
